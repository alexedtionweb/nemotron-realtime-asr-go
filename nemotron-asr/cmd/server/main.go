// Command server exposes the Nemotron streaming recognizer to a browser.
//
// Visit http://localhost:8081, allow microphone access, and the page sends
// 16 kHz mono PCM16 frames to /ws/transcribe.  The server returns JSON
// partial transcripts after each model-sized audio window.
package main
...
// Keep the server on the same locally cached model/runtime as the CLI. The
// browser server is intended to be started with `go run ./cmd/server`, with
// no model-path command-line setup.
const (
	defaultModelDir = "/home/alex/.cache/huggingface/hub/models--tonythethompson--Nemotron-3.5-ASR-Streaming-0.6B-ONNX/snapshots/3a2c8c48cf093c890740577d9467f835ca95fdbd/"
	defaultORTLib   = "/usr/local/lib/libonnxruntime.so"
	listenAddr      = ":8081"
)
...
// modelPool keeps fully loaded models alive after a browser stops recording.
// Loading the two ONNX graphs takes several seconds; Reset makes an idle model
// safe for the next utterance without paying that startup cost again.
...
func (p *modelPool) release(model *asr.Model) {
	...
	model.Reset()
	p.mu.Lock()
	// One warm model removes startup latency for the next session without
	// retaining multiple 0.6B graphs just because a user changed languages.
	if len(p.idle) == 0 {
...
func main() {
	cfg, err := loadServerConfig(defaultModelDir, defaultORTLib)
	if err != nil {
		log.Fatal(err)
	}
	...
	// The UI defaults to auto-detection. Pre-warming it means the first
	// microphone session is ready immediately instead of waiting for ONNX graph
	// construction after the user starts speaking.
	...
	app.Get("/languages", func(c fiber.Ctx) error {
		return c.JSON(availableLanguages(cfg.model.PromptDictionary))
	})
	...
	app.Get("/ws/transcribe", websocket.New(func(conn *websocket.Conn) {
		handleTranscription(conn, pool)
	}))

	log.Printf("open http://localhost%s", listenAddr)
...
func availableLanguages(prompts map[string]int64) []string {
	// Multiple aliases can map to the same prompt. Keep one concise spelling
	// for each prompt while retaining locale-only choices such as zh-CN.
